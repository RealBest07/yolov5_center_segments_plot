    ...
    def plot_boxes(self,results,frame,height,width,confidence=0.3):
        ...
        for i in range(n):
            ...
            if row[4] >= confidence:
                ...
                if self.class_to_lable(labels[i]) == 'person':
...
while cap.isOpened():
    ...
    for track in tracks:
        ...
        cv2.rectangle(img,(int(bbox[0]),int(bbox[1])),(int(bbox[2]),int(bbox[3])),(0,0,225),2)
